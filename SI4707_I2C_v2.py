...
class SI4707 :
	...
	#  Returns the current Interrupt Status.
        ...
        # Returns the current AGC Status.
        def getAGCStatus(self):
                self.writeCommand(self.WB_AGC_STATUS)
                self.response = self.i2c.readList(0, 2)
                self.agcStatus = self.response[1] 
                print "AGC STATUS:", self.agcStatus
                return self.agcStatus
	

	# Overides the AGC function
	
	def setAGCStatus(self, setting):
		self.i2c.write8(self.WB_AGC_OVERRIDE, setting)
		return
