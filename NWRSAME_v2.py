...
syslog.syslog('Starting up the Si4707')
time.sleep(1)
...
config.read('nwrsame.cfg')
print("Config read")
tuneFrequency = int(config.get("radio", "frequency"))
#radio.tuneDirect(162400)  #  Change to local frequency. 6 digits only.
radio.tuneDirect(tuneFrequency)
...
#*********************************
		...
		if (eomCnt > 2):
			# I think this code runs when there is a SAME event. -M.Englehorn
			eomCnt = 0
...
#***************Example of 15 sec relay timer setup once a relayTrigger is activated*****************#
			...
			syslog.syslog('Starting relay Timer')
...
def getStatus():
		...
		syslog.syslog('FREQ:, radio.frequency,  RSSI:, int(radio.rssi-107), dBm,  SNR:, int(radio.snr), dBuV')
        ...
        if (radio.intStatus & radio.SAMEINT):
				...
				sqlhost = config.get("mysql", "hostname")
				sqluser = config.get("mysql", "username")
				sqlpass = config.get("mysql", "password")
				sqldb = config.get("mysql", "database")
				postcmd = config.get("command", "script")

				print("Connecting to database")
				cnx = mysql.connector.connect(user=sqluser, password=sqlpass, host=sqlhost, database=sqldb)
				print("Database connected!")
				cursor = cnx.cursor()
				print("Cursor has!")
				add_event = ("INSERT INTO wxevents (originator, event, locations, `location codes`, duration, day, time, callsign, uuid) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)")
				print(add_event)
				origName = ''.join(radio.sameOriginatorName)
				eventName = ''.join(radio.sameEventName)
				sameLocations = str(int(radio.sameLocations))
				locationCodes = ''.join(radio.sameLocationCodes)
				sameDuration = ''.join(radio.sameDuration)
				sameDay = ''.join(radio.sameDay)
				sameTime = ''.join(radio.sameTime)
				sameCallSign = ''.join(radio.sameCallSign)
				data_event = (origName.rstrip(), eventName.rstrip(), sameLocations.rstrip(), locationCodes.rstrip(), sameDuration.rstrip(), sameDay.rstrip(), sameTime.rstrip(), sameCallSign.rstrip(), uuid_str)
				print(data_event)
				print("MySQL Ready!")
				cursor.execute(add_event, data_event)
				print("MySQL executed!")
				cnx.commit()
				print("MySQL Committed")
				cursor.close()
				cnx.close()
				print("MySQL Closed")

				# Execute script after connection
				cmd = [postcmd, uuid_str]
				...
				# This is what gets executed when a message comes in - MENGLEHORN
...
def sendAlert(alert):
	...
	body = MIMEText(alert)
	emsg.attach(body)
	server = smtplib.SMTP("localhost", 25) #gmail is smtp.gmail.com, 587
       	#server.starttls()
